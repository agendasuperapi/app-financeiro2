...
const AddedByField: React.FC<AddedByFieldProps> = ({
  form
}) => {
  const {
    t
    ...
    const loadNames = async () => {
      try {
        ...
        const {
          data,
          error
        } = await (supabase as any).from('view_cadastros_unificados').select('id, primeiro_name, phone').not('primeiro_name', 'is', null);
        if (error) throw error;

        // Extrair dados únicos e filtrar valores vazios
        const uniqueUsers = Array.from(new Map((data as any[]).filter(item => item.primeiro_name && item.primeiro_name.trim() !== '').map(item => [item.primeiro_name, {
    ...
    field
          ...
          <Popover open={open} onOpenChange={setOpen}>
            ...
            <PopoverContent className="w-full p-0 z-50 border bg-popover text-popover-foreground shadow-md">
              <Command>
                <CommandInput placeholder="Buscar nome..." value={field.value || ''} onValueChange={value => {
            field.onChange(value);
                ...
                <CommandList>
                  ...
                  <CommandGroup>
                    {users.map(user => <CommandItem key={user.name} value={user.name} onSelect={currentValue => {
                ...
                setOpen(false);
                        ...
                        <Check className={cn("mr-2 h-4 w-4", field.value === user.name ? "opacity-100" : "opacity-0")} />
                        {user.name}
                      </CommandItem>)}
                  </CommandGroup>
                </CommandList>
