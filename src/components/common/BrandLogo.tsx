    ...
    <div className={`flex items-center space-x-2 ${className}`}>
      {logoUrl && !imageError ? (
        <div className="relative">
          {!imageLoaded && <Skeleton className={`absolute inset-0 ${sizeClasses[size]} rounded-lg`} />}
          <img
            src={logoUrl}
        ...
        <div className={`${sizeClasses[size]} bg-primary rounded-lg flex items-center justify-center flex-shrink-0`}>
          <span className="text-primary-foreground font-bold text-sm">{companyName?.charAt(0) || "P"}</span>
        </div>
      )}

      {showCompanyName && (
        <span className={`${textSizeClasses[size]} font-bold text-primary`}>{companyName || "App Financeiro!"}</span>
      )}
    </div>
  );
};
