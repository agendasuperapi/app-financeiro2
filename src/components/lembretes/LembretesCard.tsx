...
import { Calendar, Edit, Trash2, CheckCircle, Clock, AlertTriangle, Bell } from 'lucide-react';
...
const LembretesCard: React.FC<LembretesCardProps> = ({
  transaction,
  onEdit,
  onDelete,
  onMarkAsPaid
  ...
  const getCardStyle = () => {
    if (isCompleted) return 'border-l-green-500 bg-green-50/30 dark:bg-green-900/20';
  ...
  const getStatusBadgeVariant = () => {
    ...
    if (isUpcoming) return 'bg-orange-100 text-orange-800 border-orange-200';
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            <div className={`flex items-center gap-2 ${isMobile ? 'mb-1' : 'mb-2'}`}>
              ...
              <Badge className={cn("text-xs border", getStatusBadgeVariant())}>
                {getStatusText()}
              </Badge>
            </div>
            ...
            <div className={`flex ${isMobile ? 'flex-col gap-2' : 'items-center justify-between'}`}>
              <div className={`text-muted-foreground ${isMobile ? 'text-sm' : 'text-base'}`}>
                Lembrete - {transaction.description}
              </div>
              
              <div className={`flex items-center ${isMobile ? 'gap-1' : 'gap-2'}`}>
                {onMarkAsPaid && !isCompleted && (
                  <Button
                    size={isMobile ? "sm" : "sm"}
                    variant="outline"
            ...
            {isCompleted && transaction.paidDate && (
              <div className={`mt-2 text-green-600 flex items-center gap-1 ${isMobile ? 'text-xs' : 'text-xs'}`}>
                <CheckCircle className="h-3 w-3" />
                Concluído em {format(new Date(transaction.paidDate), 'dd/MM/yyyy', { locale: ptBR })}
              </div>
            )}
          </div>
        </div>
