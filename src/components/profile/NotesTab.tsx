  ...
  const handleAddNote = async (values: NoteFormValues) => {
    try {
      if (isClientView && selectedUser) {
        ...
        const { error } = await (supabase as any).from('financeiro_notas').insert({
    ...
    <SubscriptionGuard feature="notas ilimitadas">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold">Minhas Notas</h3>
            <p className="text-sm text-muted-foreground">Gerencie suas anotações pessoais</p>
          </div>
