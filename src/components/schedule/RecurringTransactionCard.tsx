...
const RecurringTransactionCard: React.FC<RecurringTransactionCardProps> = ({
  ...
  return <Card className={cn("transition-all hover:shadow-md border-l-4", getCardStyle())}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className={`flex ${isMobile ? 'flex-col gap-2' : 'items-center justify-between'}`}>
              ...
              <div className={`flex items-center ${isMobile ? 'gap-1' : 'gap-2'}`}>
                ...
                <Button size={isMobile ? "sm" : "sm"} variant="ghost" onClick={() => onEdit(transaction)} className={isMobile ? 'h-7 px-2' : 'h-8 px-2'}>
                  <Edit className={isMobile ? 'h-3 w-3' : 'h-3 w-3'} />
                </Button>
                
                <Button size={isMobile ? "sm" : "sm"} variant="ghost" onClick={() => onDelete(transaction.id)} className={`text-red-600 hover:bg-red-50 ${isMobile ? 'h-7 px-2' : 'h-8 px-2'}`}>
                  <Trash2 className={isMobile ? 'h-3 w-3' : 'h-3 w-3'} />
                </Button>
              </div>
            </div>

            {isPaid && transaction.paidDate && <div className={`mt-2 text-green-600 flex items-center gap-1 ${isMobile ? 'text-xs' : 'text-xs'}`}>
