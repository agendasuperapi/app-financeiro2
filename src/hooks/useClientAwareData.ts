...
export const useClientAwareData = () => {
  ...
  const fetchClientTransactions = useCallback(async () => {
    ...
    try {
      ...
      // Fetch timezone for this user (cached)
      ...
      return data.map((transaction: any) => ({
        ...
        creatorName: transaction.name || undefined,
  ...
  // Removido: sincronização automática que causava recarregamentos excessivos

  // Realtime Supabase otimizado: debounce para evitar múltiplas atualizações
  useEffect(() => {
    if (!isClientView || !targetUserId) return;
    ...
    const channel = (supabase as any)
      ...
      .on('postgres_changes', { 
        ...
        debounceTimer = setTimeout(() => loadClientData(), 300);
      })
      .subscribe();
