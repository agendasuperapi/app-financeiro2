...
export const markAsPaid = async (
  ...
  try {
    // Como não existe campo status na tabela e o usuário não quer [PAGO] na descrição,
    // esta função não fará alterações no banco por enquanto
    console.log(`Transaction ${transactionId} marked as paid (UI only)`);
    return true;
  } catch (error) {
